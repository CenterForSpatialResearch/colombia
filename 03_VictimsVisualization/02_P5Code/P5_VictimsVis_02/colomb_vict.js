// =======================================================================
// * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~
// ~ * ~ * ~ INTERACTIVE COLOMBIA VICTIMS VISUALIZATION  * ~ * ~ * ~ * ~ * 
// * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~ * ~

// Implementing Processing Victims Visualization in p5. A visualization
// that shows a random sample of victims in the Colombia conflict where
// each data point represents one victim. Users can sort and filter the
// data by a number of parameters.
...
function preload() {
    ...
    victimsTable = loadTable("data/MH_100k_test_02.csv", "header");
}
...
function setup() {
    ...
    // var numberOfRecords = 200000;

    println("victims Table check " + victimsTable)
    println("number of records: " + numberOfRecords);
    ...
    for (var rowIndex = 0; rowIndex < numberOfRecords; rowIndex++) {
        ...
        if (String(row.getString("genero_hom")) == "Hombre"){
            genero = "Male";
        }
        else if (String(row.getString("genero_hom")) == "Mujer"){
            genero = "Female";
        }
        else{
            genero = "Other or N/A";
        }
        if (String(row.getString("pert_etnica")) == "Negro(a) o Afrocolombiano(a)"){
            ethnicity = "Afrocolombian";
        }
        else if (String(row.getString("pert_etnica")) == "Gitano(a) ROM"){
            ethnicity = "ROM";
        }
        else if (String(row.getString("pert_etnica")) == "Indigena"){
            ethnicity = "Indigenous";
        }
        else if (String(row.getString("pert_etnica")) == "Palenquero"){
            ethnicity = "Palenquero";
        }
        else if (String(row.getString("pert_etnica")) == "Raizal del Archipielago de San Andres y Providencia"){
            ethnicity = "San Andres and Providencia";
        }
        else{
            ethnicity = "None";
        }
        paramHecho = parseInt(row.getString("param_hecho"));
        ...
        daneOcurrencia = parseInt(row.getString("dane_ocurrencia"));
        ...
        if (paramHecho == 5){
            ...
            daneLlegada = row.getNum("dane_llegada");
        ...
        daneDeclaracion = parseInt(row.getString("dane_decla"));
        daneUbicacion = parseInt(row.getString("dane_ub"));
        ...
        if (parseInt(row.getString("autor_guerrillas")) == 0){
            autorGuerrillas = false;
        }
        else{
            autorGuerrillas = true;
        }
        if (parseInt(row.getString("autor_paramilitares")) == 0){
            autorParamilitares = false;
        }
        else{
            autorParamilitares = true;
        }
        if (parseInt(row.getString("autor_fuerza_publica")) == 0){
            autorFuerzaPublica = false;
        }
        else{
            autorFuerzaPublica = true;
        }
        if (parseInt(row.getString("autor_bacrim")) == 0){
            autorBacrim = false;
        }
        else{
            autorBacrim = true;
        }
        if (parseInt(row.getString("autor_otros")) == 0){
            autorOtros = false;
        }
        else{
            autorOtros = true;
        }
        if (parseInt(row.getString("autor_no_identifica")) == 0){
            autorNoId = false;
        }
        else{
            autorNoId = true;
...
function drawVictims(){
    ...
    // println("record length in draw Victims: " + record.length);
    ...
    for (var i = 0; i < record.length; i++){
        if (record[i].active){
            ...
            var locationCity = eventLocation.split("");
...
function setActive(){
    if (frameCount % changeRate == 0){
        if (bogota){
            largeList = [];
            for (var i = 0; i < record.length; ++i) {
                if (String(record[i].cityType) == "large"){
                    largeList.push(i);
                } else {
                }
            }
            ...
            smallList = [];
            for (var i = 0; i < record.length; ++i) {
                if (String(record[i].cityType) == "small"){
                    smallList.push(i);
                } else {
                }
            }
...
function draw() {
    ...
    //Test how long it takes to run through each draw
    var start = millis();
...
function Victims(actor_event, param_hecho, ocurrencia_year, ocurrencia_month, ocurrencia_day, dane_ocurrencia, reporte_year, reporte_month, reporte_day, daneDeclaracion, llegadaYear, llegadaMonth, llegadaDay, daneUbicacion, dane_llegada, tipoDesplazamiento, tipoDeVictima, mpio_ocurrencia, mpioLlegada, mpioUbicacion, mpioDeclaracion, autor_guerrilla, autor_paramilitares, autor_fuerzaPublica, autor_bacrim, autor_otros, autor_noId, start_x, start_y, _genero, _ethnicity, date_event, date_report, city_type, sort_type)
{
    ...
    this.reportMonth = this.listMonths[reporte_month - 1];
    this.active = false;
  ...
  this.simplePlot = function() {
    ...
    if (parametros){
      ...
      if (String(this.genero) == "Male"){
        fillColor = color1;
      }
      else if (String(this.genero) == "Female"){
        fillColor = color2;
      }
      else if (String(this.genero) == "Other or N/A"){
        fillColor = color3;
      }
      else{
        fillColor = color4;
      ...
      if (String(this.ethnicity) == "None"){
        fillColor = color1;
      }
      else if (String(this.ethnicity) == "Afrocolombian"){
        fillColor = color2;
      }
      else if (String(this.ethnicity) == "Indigenous"){
        fillColor = color3;
      }
      else{
        fillColor = color4;
    ...
    // }
    fill(fillColor);
    rect(this.endX, this.endY, 2.5, 2.5);
    }
}
// }
